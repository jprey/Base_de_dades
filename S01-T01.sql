/* ============================================================================ */
/* EX 1: A partir dels documents adjunts (estructura i dades), crea una base	*/
/* de dades amb MySQL. Mostra les característiques principals de l'esquema  	*/
/* creat i explica les diferents taules i variables que hi ha.                  */
/* ============================================================================ */

CREATE DATABASE S01_T01; /* Creem la base de dades S01_T01 com una agrupacio de esquemes */
CREATE SCHEMA movies;	/* Creem el SCHEMA movies com una coleccio de taules */
USE movies;
SHOW DATABASES;
...
-- ---------------------------------------------------------------------------
-- INSERT INTO movies.tb_movie_person
-- --------------------------------------------------------------------------- 
...
/* ============================================================================ */
/* EX 2: Realitza la següent consulta sobre la base de dades acabada de crear:	*/
...
SELECT person_name, person_country, person_dob FROM tb_person WHERE person_dod IS NULL ORDER BY person_dob DESC;

/* ============================================================================ */
/* EX 3: Realitza la següent consulta sobre la base de dades acabada de crear:	*/
...
SELECT tb_genre.genre_name, COUNT(tb_genre.genre_name) FROM tb_genre
INNER JOIN tb_movie ON tb_movie.movie_genre_id = tb_genre.genre_id
GROUP BY tb_genre.genre_name ORDER BY COUNT(tb_genre.genre_name) DESC;

/* ============================================================================ */
/* EX 4: Realitza la següent consulta sobre la base de dades acabada de crear:	*/
/* Has d'obtenir, per a cada persona, el seu nom i el nombre màxim de rols 		*/
/* diferents que ha tingut en una mateixa pel·lícula.							*/
/* Posteriorment, mostra únicament aquelles persones que hagin assumit més d'un */
/* rol en una mateixa pel·lícula. 											 	*/
/* ============================================================================ */

SELECT DISTINCT person_name, COUNT(DISTINCT role_id) FROM tb_person, tb_movie_person
WHERE tb_person.person_id = tb_movie_person.person_id 
GROUP BY person_name HAVING COUNT(DISTINCT role_id)>1;

/* ============================================================================ */
/* EX 5: Realitza la següent operació sobre la base de dades acabada de crear:	*/
...
/* ============================================================================ */
/* EX 6: Fes la següent operació sobre la base de dades acabada de crear: 		*/
...
DELETE FROM tb_movie WHERE tb_movie.movie_title = 'La Gran Familia Española';
...
/* ============================================================================ */
/* EX 7: Realitza la següent operació sobre la base de dades acabada de crear: 	*/
/* Canvia el gènere de la pel·lícula "Ocho apellidos catalanes" perquè consti 	*/
/* com a comèdia i no com a romàntica.	    									*/
/* ============================================================================ */

SELECT * FROM movies.tb_genre;
SELECT genre_name, genre_id FROM tb_genre
WHERE genre_name = "Comedia";

SET SQL_SAFE_UPDATES = 0;
UPDATE movies.tb_movie SET movie_genre_id=3
WHERE movie_title="Ocho apellidos catalanes";
SET SQL_SAFE_UPDATES = 1;

SELECT * FROM movies.tb_movie;

SELECT movie_title, tb_genre.genre_name FROM tb_movie, tb_genre
WHERE tb_genre.genre_id = movies.tb_movie.movie_genre_id;
